...
SkinningTest::SkinningTest(HINSTANCE hInstance)
	: MainApp(hInstance)
...
bool SkinningTest::Initialize(UINT Width, UINT Height)
{
...
void SkinningTest::Render()
{
	...
	// Draw계열 함수를 호출하기전에 렌더링 파이프라인에 필수 스테이지 설정을 해야한다.	
	{
		...
		//m_pDeviceContext->PSSetConstantBuffers(3, 1, &m_pMatrixPalleteCB);
...
bool SkinningTest::InitScene()
{
	...
	{
		HR_T(CompileShaderFromFile(L"SkinningTest_VS.hlsl", "main", "vs_5_0", &vertexShaderBuffer));
	...
	/// 5. Render() 에서 파이프라인에 바인딩할 픽셀 셰이더 생성
	{
		...
		HR_T(CompileShaderFromFile(L"SkinningTest_PS.hlsl", "main", "ps_5_0", &pixelShaderBuffer));
...
LRESULT CALLBACK SkinningTest::WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
