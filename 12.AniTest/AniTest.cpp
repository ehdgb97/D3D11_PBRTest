...
AniTest::AniTest(HINSTANCE hInstance)
	: MainApp(hInstance)
...
bool AniTest::Initialize(UINT Width, UINT Height)
{
...
bool AniTest::InitScene()
{
	...
	{
		HR_T(CompileShaderFromFile(L"AniTest_VS.hlsl", "main", "vs_5_0", &vertexShaderBuffer));
	...
	/// 5. Render() 에서 파이프라인에 바인딩할 픽셀 셰이더 생성
	{
		...
		HR_T(CompileShaderFromFile(L"AniTest_PS.hlsl", "main", "ps_5_0", &pixelShaderBuffer));
...
//}
	for (int i = 0; i < SpawnObject; i++)
	{
		...
		newObj->SetFBX(m_pDevice, "Box_Walk.fbx");
...
LRESULT CALLBACK AniTest::WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
