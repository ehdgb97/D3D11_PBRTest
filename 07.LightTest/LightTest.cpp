...
struct Vertex
{
    ...
    Vertex(Vector3 position, Vector3 Normal)
        : position(position), Normal(Normal), color(.0f, .0f, .0f,1.f) { }

    Vertex(Vector3 position, Vector3 Normal, Vector4 color)
        : position(position),Normal(Normal), color(color) { }
};



LightTest::LightTest(HINSTANCE hInstance)
...
bool LightTest::Initialize(UINT Width, UINT Height)
{
...
void LightTest::Update()
{
    ...
    ///라이트 계산
    {
        ...
        //XMMATRIX mRotate = XMMatrixRotationY(-2.0f * GameTimer::m_Instance->DeltaTime());
    ...
    ///큐브 셋팅 모음.
	{
        ...
        ///
		{
            ///1번 큐브 셋팅
			{
				Matrix scaleMatrix = DirectX::XMMatrixScaling(m_cubesScale.x, m_cubesScale.y, m_cubesScale.z);
				Matrix rotationMatrix = XMMatrixRotationY(cubeRotation);
...
#pragma region Imgui
    ...
    // 2. Show a simple window that we create ourselves. We use a Begin/End pair to create a named window.
    {
        ...
        if (ImGui::CollapsingHeader("Cube Position Setting"))
        {
            ...
            ImGui::SameLine(155);
            ImGui::SliderFloat("##SliderCubeScale", reinterpret_cast<float*>(&m_cubesScale), 0.f, 2.f);
            ImGui::Text("");    ImGui::SameLine(155);
            ImGui::InputFloat("##InputCubeScale", reinterpret_cast<float*>(&m_cubesScale));
            ImGui::Text("");
        ...
        if (ImGui::CollapsingHeader("Light Setting"))
        {
            ...
            ImGui::SameLine(155);
            ImGui::SliderFloat3("##SliderLightDir", reinterpret_cast<float*>(&m_InitialLightDirs),-1.f,1.f);
            ImGui::Text("");    ImGui::SameLine(155);
            ImGui::InputFloat3("##InputLightDir", reinterpret_cast<float*>(&m_InitialLightDirs));
...
bool LightTest::InitScene()
{
	...
	ID3DBlob* vertexShaderBuffer = nullptr;
    {
        ...
        HR_T(CompileShaderFromFile(L"LightTest_VS.hlsl", "main", "vs_4_0", &vertexShaderBuffer));
    ...
    /// 5. Render() 에서 파이프라인에 바인딩할 픽셀 셰이더 생성
    {
        ...
        HR_T(CompileShaderFromFile(L"LightTest_PS.hlsl", "main", "ps_4_0", &pixelShaderBuffer));
...
LRESULT CALLBACK LightTest::WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
